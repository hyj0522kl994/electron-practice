...
export default class Radios extends React.Component{
  ...
  handleOnSubmit(e) {
    ...
    newRoomRef.update(newRoom).then(() => {
      ...
      return this.fetchRooms().then(() => {
        ...
        hashHistory.push(`/radios/${newRoomRef.key}`);
      });
    });
  }
  // 채팅방 목록 추출 처리
  fetchRooms() {
    ...
    return this.db.ref("/chatrooms").limitToLast(20).once("value").then(snapshot => {
      ...
      snapshot.forEach(item => {
        ...
        radios.push(Object.assign({ key: item.key }, item.val()));
      });
  ...
  renderRoomList() {
    ...
    const { radios, roomName } = this.state;
    return (
      <div className="list-group">
        {radios.map(r => <Channels room={r} key={r.key} selected={r.key === roomId} /> )}
